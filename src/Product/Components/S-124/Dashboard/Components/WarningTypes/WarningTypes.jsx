...
function WarningTypes() {
    ...
    useEffect(() => {
        if (staticTableref.current) {
            const columns = [
                ...
                { title: 'Warning Type', className: 'text-start w-20', data: 'warningType' },
    ...
    const fetchAllTemplateData = async () => {
        try {
            const response = await axios.get(`${s124_NavWarn_Apis.getWarningTypes}`);
                    ...
                    <Table responsive bordered striped className='mb-0 w-100' ref={staticTableref}>
                        <thead>
                            <tr>
                                ...
                                <th className='text-center w-20'>Warning Type</th>
