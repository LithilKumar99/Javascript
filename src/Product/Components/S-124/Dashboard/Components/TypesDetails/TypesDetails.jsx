...
function TypesDetails() {
    ...
    useEffect(() => {
        if (staticTableref.current) {
            const columns = [
                ...
                { title: 'Navwarn Type', className: 'text-start w-20', data: 'navwarnType' },
    ...
    const fetchAllTemplateData = async () => {
        try {
            const response = await axios.get(`${s124_NavWarn_Apis.getTypeDetails}`);
                    ...
                    <Table responsive bordered striped className='mb-0 w-100' ref={staticTableref}>
                        <thead>
                            <tr>
                                ...
                                <th className='text-center w-20'>Navwarn Type</th>
